...
- `ClientRouter` never listens for `pageshow`, so it never re-fires `astro:page-load` when the browser restores a page from the back/forward cache (bfcache) — common on Safari when navigating back then forward, or reopening a backgrounded tab. A page that's just static markup re-hydrated from DOM state doesn't care, but one driving continuous canvas/animation state (`play.astro`'s `requestAnimationFrame` loop) can be left exactly as the browser froze it. `play.astro` covers this with its own `window.addEventListener("pageshow", (event) => { if (event.persisted) initLife(); })` rather than a shared mechanism — no other page currently has this class of state, so there's nothing yet to generalize.
